...
module('Keyboard Select Picker Integration', {
  beforeEach: function() {
    App = startApp();
  },
  afterEach: function() {
...
test('Show and Hide', function(assert) {
  ...
  .keyEvent('#single-picker .keyboard-select-picker', 'keyup', 27) // Escape

  .then(function() {
    assert.ok(
      find('#single-picker .dropdown-menu').is(':hidden'),
      'dropdown menu should be hidden after pressing escape'
    );
  });
});
